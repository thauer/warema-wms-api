...
function decodeStickCmd( rcv ){
	logger.silly( "wmsUtil: decodeStickCmd: " + rcv );
	...
	if( rcv.startsWith( '{a}' ) ){
		...
		// AAAAAA = SNR (Serial Number) from the interrogated adapter in HEX.
		// 8011 = Nachrichten Typ hier Statusantwort // Message type here status response
		// PP = Position in % * 2 (HEX) must be divisible by 2
		// WW = Winkel +127 in HEX. 127 corresponds to 0°.
		// V1 = Position Volant 1. FF entspricht nicht vorhanden. // FF: not available
		// V2 = Position Volant 2. FF entspricht nicht vorhanden. // FF: not available
		...
		switch( rcvTyp ){
			...
			case '7080':
				...
				params.illuminance = payload.substr(4, 2) === '00' ? parseInt(payload.substr(12, 2), 16) * 2 : parseInt(payload.substr(4, 2), 16) * parseInt(payload.substr(12, 2), 16) * 2;
				params.unknown_2 = payload.substr(6, 6);
				params.unknown_3 = payload.substr(14, 2);
				params.rain = payload.substr(16, 2) === 'C8';
				params.temperature = parseInt(payload.substr(18, 2), 16) / 2 - 35;
	...
	logger.silly( 'wmsUtil: Decoded ' + JSON.stringify(ret) + '.' );
	return ret;
}
