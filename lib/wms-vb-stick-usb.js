...
class WmsVbStickUsb extends WmsVbStick {
  constructor( portPath, channel, panid, key, optionsPar, callback ){
		logger.info("Setting up WmsVbStickUsb( " + 
		portPath + ", " + channel + ", " + panid + ", " + key + " )")
	...
	openUsbPort( portPath, channel, panid, key, options ){
		...
		this.port.on('open', function() {
			logger.debug( 'Opened port ' + portPath + ' and listening ...' );
			stickObj.initWmsNetwork();
		})

		this.port.on('error', function(err) {
			if( !portPath ){ portPath = portPath; };
			logger.error( portPath+' error: ', err.message)
			this.status = "error";
		})

		/**
		 * Callback function that the USB antenna uses to handle raw events 
		 */
		this.parser.on('data', function (data) {
			logger.silly( "UsbPort RCV <<< " + data.toString('utf8'));
			stickObj.comDataReceive( data.toString('utf8') + DelimiterChar );
		});

		this.parser.on('close', function (data) {
			logger.info( portPath + ' port closed. ' );
			this.status = "created";
		});
	}

	close() {
		logger.info("closeUsbPort()")
		this.port.close();
	}

	// Overwriting suber class
	comDataSendCallback( dataString ){
		logger.silly("comDataSendCallBack(" + dataString + ")");
		// . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
